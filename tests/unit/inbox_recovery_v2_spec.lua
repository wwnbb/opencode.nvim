...
describe("v2 inbox/history recovery", function()
	...
	it("keeps an admitted prompt outside complete native history", function()
		...
		assert.equals("Queued", selectors.prompt_status("s", "q"))
	end)
	...
	it("remembers delivery from history before a late admission arrives", function()
		start()
		callbacks.history(nil, projection.page("s", { { id = "q", type = "user", text = "Delivered", time = { created = 30 } } }))
		callbacks.inbox(nil, {})
		pending.admit(item())
		bus.emit("v2_event", { id = "evt_old", created = 10, type = "session.inbox.enqueued",
			data = { sessionID = "s", inboxID = "q", item = item() } })
		assert.equals("delivered", pending.get("s", "q").status)
		assert.equals("Delivered", sync.get_parts("q")[1].text)
		assert.equals(30, sync.get_message("s", "q").time.created)
		assert.is_nil(selectors.prompt_status("s", "q"))
	end)

	it("clears the badge on delivery while later admission and inbox reads are in flight", function()
		bus.emit("v2_event", { id = "evt_queue", created = 10, type = "session.inbox.enqueued",
			data = { sessionID = "s", inboxID = "q", item = item() } })
		assert.equals("Queued", selectors.prompt_status("s", "q"))
		start()
		bus.emit("v2_event", { id = "evt_delivery", created = 20, type = "session.inbox.delivered",
			data = { sessionID = "s", inboxID = "q" } })
		pending.admit(item())
		callbacks.history(nil, {}); callbacks.inbox(nil, { item() })
		assert.is_nil(selectors.prompt_status("s", "q"))
		assert.is_nil(selectors.pending_input("s", "q"))
		assert.equals("Queued text", sync.get_parts("q")[1].text)
	end)
